...
namespace DiscordBeatSaberBot
{
    internal static class BeatSaberInfoExtension
    {
        ...
        public static async Task<EmbedBuilder> GetTop10Players()
        {
            ...
            {
                string infoToTell = "";
                foreach (string result in player)
                {
                    string item = result.Replace(@"\r\n", " ").Trim();
        ...
        public static async Task<List<EmbedBuilder>> GetPlayer(string playerName)
        {
            ...
            foreach (var player in players)
            {
                ...
                string ppNext = "-";
                string ppBefore = "-";
                string playerNextName = "Not Found o.o";
                try
                {
                    double ppNextDouble = Math.Round(double.Parse(player.Next.pp) - double.Parse(player.pp), 0);
                    double ppBeforeDouble = Math.Round(double.Parse(player.pp) - double.Parse(player.Before.pp), 0);
        ...
        {
            string url = "https://scoresaber.com/u/" + scoresaberId;
            ...
            using (var client = new HttpClient())
            {
                ...
                string playerName = doc.DocumentNode.SelectSingleNode("//h5[@class='title is-5']").InnerText.Replace("\n", "").Replace("\r", "").Trim();
        ...
        public static async Task<List<EmbedBuilder>> GetSongs(string search)
        {
            ...
            string url = "https://beatsaver.com/search/all/0?key=" + search.Replace(" ", "+");
        ...
        public static async Task<EmbedBuilder> GetTopSongList(string search)
        {
            ...
            string topSongUrlString = "";
            var topSongImg = new List<List<string>>();
            string topSongImgString = "";
            var topList = new List<List<string>>();
            string url = "https://scoresaber.com/top";
            using (var client = new HttpClient())
            {
                string html = await client.GetStringAsync(url);
                var doc = new HtmlDocument();
            ...
            string songName = TopSongInfo[0].Replace("\r\n", "").Trim();
            string songDifficulty = TopSongInfo[2];
            string songStar = TopSongInfo[4];
            string songPlays = TopSongInfo[3];
        ...
        public static async Task<string> GetImageUrlFromId(string playerId)
        {
            ...
            using (var client = new HttpClient())
            {
                string url = "https://scoresaber.com" + playerId;
                string html = await client.GetStringAsync(url);
                var doc = new HtmlDocument();
                doc.LoadHtml(html);
        ...
        {
            string playerTopSongImg = "";
            string playerTopSongLink = "";
            string playerTopSongName = "";
            string playerTopSongPP = "";
            string playerTopSongAcc = "";
            string songName = "";
        ...
        public static async Task<EmbedBuilder> GetRecentSongWithId(string playerId)
        {
            ...
            string songAuthor = "";
            string playerName = "";
            string playerSongRank = "";

            string url = "https://scoresaber.com/u/" + playerId.Replace("/u/", "") + "&sort=2";
            using (var client = new HttpClient())
            {
                string html = await client.GetStringAsync(url);
                var doc = new HtmlDocument();
        ...
        public static async Task<(string, string)> GetRecentSongInfoWithId(string playerId)
        {
            ...
            string url = "https://scoresaber.com" + playerId + "&sort=2";
            using (var client = new HttpClient())
            {
                string html = await client.GetStringAsync(url);
                var doc = new HtmlDocument();
        ...
        public static async Task<EmbedBuilder> GetTopxCountry(string search)
        {
            ...
            if (input.Count() == 1)
            {
                ...
                int countryrank = player.First().countryRank;
                string countryName = player.First().countryName;
            ...
            for (int x = 1; x <= tab; x++)
            {
                string url = "https://scoresaber.com/global/" + x + "&country=" + input[0];
            ...
            for (int x = 0; x < int.Parse(input[1]); x++)
            {
                string add = Names[x].Replace("\r\n", " ").Replace("&nbsp&nbsp", "");
                add = add.Trim();
                topx.Add(add);
            }
            ...
            string playerName)
        {
            ...
            double tab = Math.Ceiling(t);
            double rankOnTab = countryRank % 50;
            double count = countryRank;
            var Names = new List<string>();
            var namesTop = new List<string>();
            var namesBottom = new List<string>();

            string url = "https://scoresaber.com/global/" + tab + "&country=" + country;
            await GetNames(url);
                ...
                int otherPage = 0)
            {
                using (var client = new HttpClient())
                {
                    string html = await client.GetStringAsync(infoUrl);
                    var doc = new HtmlDocument();
                    doc.LoadHtml(html);
            ...
            foreach (string rank in outputList)
            {
                if (counter > Rank - 4 && counter < Rank + 4)
                {
                    if (counter == Rank)
                    {
                        string name = rank.Replace("\r\n", " ").Replace("&nbsp&nbsp", "").Trim();
        ...
        public static async Task<(string, string)> RankedNeighbours(string playerName, int playerRank, int recursionLoop = 0)
        {
            ...
            double t = Rank / 50;
            double tab = Math.Ceiling(t);
            double rankOnTab = Rank % 50;
            double count = Rank;
            var Names = new List<string>();
            var namesTop = new List<string>();
            var namesBottom = new List<string>();

            string url = "https://scoresaber.com/global/" + tab;
            await GetNames(url);
        ...
        public static async Task<string> GetPlayerCountryRank(string name)
        {
            ...
            string url = "https://scoresaber.com" + playerId.First();
            using (var client = new HttpClient())
            {
                string html = await client.GetStringAsync(url);
                var doc = new HtmlDocument();
        ...
        {
            string name = message.Content.Substring(12);
            ...
            (string playerName, string songName) = await GetRecentSongInfoWithId(id.First());
            ...
            if (songName == "Hi - Hi Easy")
            {
                ...
                int rank = int.Parse(rankcountry);
                if (rank == 1)
                {
        ...
        private static async Task ScrapPlayerInfo(string playerName, int recursionLoop, bool skipNeighbour, List<Player> players)
        {
            ...
            int recursionCounter = recursionLoop;

            var playerInfo = (List<List<string>>) null;
            var playerInfo2 = (List<List<string>>) null;
            var playerImg = (List<List<string>>) null;
            string url = "https://scoresaber.com/global?search=" + playerName.Replace(" ", "+");
            ...
            foreach (string id in ids)
            {
                ...
                string rank = playerInfo2.First()[0].Replace("\r\n", "").Trim();
        ...
        {
            string url = "https://scoresaber.com/global?search=" + Name;
            using (var client = new HttpClient())
            {
                string html = await client.GetStringAsync(url);
                var doc = new HtmlDocument();
                ...
                if (PP == null)
                    return "0";
                string pp = PP.InnerText.Replace("\r\n", "").Replace("Performance Points: ", "").Replace(",", "").Replace("pp", "").Trim();
                return pp.Split('.')[0];
            }
        }
        ...
        public static async Task<EmbedBuilder> PlayerComparer(string message)
        {
            ...
            int pp1 = int.Parse(player1.pp.Split(".")[0]);
            int pp2 = int.Parse(player2.pp.Split(".")[0]);
            int ppDifference = Math.Abs(pp1 - pp2);

            int totalPointsDifference = Math.Abs(int.Parse(player1.totalScore.Replace(",", "")) - int.Parse(player2.totalScore.Replace(",", "")));
