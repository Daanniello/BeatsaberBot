...
namespace DiscordBeatSaberBot.Commands
{
    ...
    {
        [Help("Help", "The help command helps you with getting to know the bot. Use the Help with command name behind it to get more info.")]
        static public async Task Help(DiscordSocketClient discordSocketClient, SocketMessage message)
        {
            if (message.Content.Length == 8)
            {
                await message.Channel.SendMessageAsync("", false, DiscordBeatSaberBot.Help.GetHelpList(discordSocketClient));
            }
        }

        [Help("Playing", "Sets the game of the discord bot.")]
        ...
        [Help("Invite", "Gives an invite link for the bot to join in other discord servers.")]
        ...
        [Help("SendDM", "Sends a DM to somone if he is in the server")]
        ...
        [Help("Poll", "Creates a poll with reactions so people can vote on a subject.")]
